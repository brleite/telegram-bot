...
def send_image(chat_id, token, image_url):
    ...
    response = requests.post('https://api.telegram.org/bot' + token + '/sendPhoto', data, verify=False)
...
def send_document(chat_id, token, document_url, file_description, parse_mode='html'):
    ...
    response = requests.post('https://api.telegram.org/bot' + token + '/sendDocument', data, verify=False)
